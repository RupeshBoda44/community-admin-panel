...
@Component({
  selector: 'app-sub-cast',
  templateUrl: './sub-cast.component.html',
  styleUrls: ['./sub-cast.component.scss']
})
export class SubCastComponent implements OnInit {
  ...
  fetchList() {
    ...
    this.http.get<any>(`${this.baseUrl}/admin/sub-cast/list`, { params }).subscribe({
      next: (res) => {
  ...
  save() {
    ...
    const req = this.drawerMode === 'add'
      ? this.http.post(`${this.baseUrl}/admin/sub-cast`, payload)
      : this.http.put(`${this.baseUrl}/admin/sub-cast/${this.selectedItem.id}`, payload);
  ...
  confirmDelete(item: any) {
    ...
    ref.afterClosed().subscribe(confirmed => {
      if (confirmed) {
        this.http.delete(`${this.baseUrl}/admin/sub-cast/${item.id}`).subscribe(() => this.fetchList());
      }
    });
  }

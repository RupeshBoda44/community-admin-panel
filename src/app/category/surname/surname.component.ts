...
export class SurnameComponent implements OnInit {
  ...
  fetchList() {
    ...
    this.http.get<any>(`${this.baseUrl}/admin/surname/list`, { params }).subscribe({
      next: (res) => {
  ...
  save() {
    ...
    const req = this.drawerMode === 'add'
      ? this.http.post(`${this.baseUrl}/admin/surname`, payload)
      : this.http.put(`${this.baseUrl}/admin/surname/${this.selectedItem.id}`, payload);
  ...
  confirmDelete(item: any) {
    ...
    ref.afterClosed().subscribe(confirmed => {
      if (confirmed) {
        this.http.delete(`${this.baseUrl}/admin/surname/${item.id}`).subscribe(() => this.fetchList());
      }
    });
  }

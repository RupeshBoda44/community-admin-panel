...
@Component({
  selector: 'app-jilla',
  templateUrl: './jilla.component.html',
  styleUrls: ['./jilla.component.scss']
})
export class JillaComponent implements OnInit {
  ...
  fetchList() {
    ...
    this.http.get<any>(`${this.baseUrl}/admin/jilla/list`, { params }).subscribe({
      next: (res) => {
  ...
  save() {
    ...
    const req = this.drawerMode === 'add'
      ? this.http.post(`${this.baseUrl}/admin/jilla`, payload)
      : this.http.put(`${this.baseUrl}/admin/jilla/${this.selectedItem.id}`, payload);
  ...
  confirmDelete(item: any) {
    ...
    ref.afterClosed().subscribe(confirmed => {
      if (confirmed) {
        this.http.delete(`${this.baseUrl}/admin/jilla/${item.id}`).subscribe(() => this.fetchList());
      }
    });
  }
